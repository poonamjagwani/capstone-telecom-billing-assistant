...
def _extract_monthly_prices(text: str) -> list[float]:
    prices: list[float] = []
    for m in re.findall(r"(?:\$|cad\s*)(\d{1,3}(?:\.\d{1,2})?)", text, flags=re.IGNORECASE):
        try:
            value = float(m)
            if 5 <= value <= 300:
                prices.append(value)
...
def web_search(query: str, max_results: int = 5) -> tuple[str, list[SourceItem]]:
    ...
    require_cheaper = "cheaper" in query.lower() or "lower" in query.lower() or "save" in query.lower()
    ...
    if not priced_hits:
        return (
            "I found results, but not enough explicit priced options to make a reliable recommendation. "
            "Try: 'Show 3 cheaper Canadian unlimited plans with exact monthly price and data caps.'",
            sources,
        )
